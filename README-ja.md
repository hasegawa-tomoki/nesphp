...
[ホスト]
  ...
  reset → compile_and_emit (vm/compiler.s)
        ...
        ▼
  PRG-RAM bank 0 ($6000-$7FFF) に op_array 完成 → main_loop で実行
```
...
| 配列 | 整数キーのみ: `$a = [1,2,3]` リテラル (ネスト可 `[[1,2],[3,4]]`)、`$a[i]` 読取 (チェーン可 `$a[i][j]`)、`$a[i] = v` 書換、`$a[] = v` 追加、`count($a)`。`foreach` / 連想 は未対応。8KB pool (PRG-RAM bank 1、1 要素 4B tagged zval)、**shared-pointer 意味論** (`$b = $a; $b[0]=99;` で `$a[0]` にも波及) |
| 文字列 | double-quoted リテラルのみ、連結 (`.`) 未対応。non-ASCII byte pass through + `\xHH` / `\\` / `\"` エスケープ対応。decoded 結果は STR_POOL (PRG-RAM bank 2、8KB) に置く |
...
| 関数 | 引数 | 説明 | opcode |
|---|---|---|---|
| `echo $v` | IS_STRING / IS_LONG | nametable の現在カーソル位置に出力 | `ZEND_ECHO` 136 (0x88) |
| `fgets(STDIN)` | — | コントローラの 1 ボタン押下を 1 文字の文字列で返す (`"A"/"B"/"S"/"T"/"U"/"D"/"L"/"R"`)、blocking | 0xF0 |
| `nes_put($x, $y, $c)` | int, int, char リテラル または runtime int | nametable (x, y) に 1 文字 (タイル番号) | 0xF1 |
...
| `nes_chr_bg($n)` | int リテラル 0-3 | PRG-ROM (CHRDATA) の 4KB CHR セット $n を BG pattern table (CHR-RAM $0000) にコピー | 0xF6 |
| `nes_chr_spr($n)` | int リテラル 0-3 | PRG-ROM (CHRDATA) の 4KB CHR セット $n を sprite pattern table (CHR-RAM $1000) にコピー | 0xF5 |
...
| `nes_attr($x, $y, $pal)` | int, int, int (runtime 可、0-3 にクランプ) | BG attribute table: 2×2 タイル (16×16 px) ブロック単位でパレット番号を割当 | 0xF9 |
...
| `nes_peek_ext($offset)` | int 0-8191 | USER_RAM_EXT (PRG-RAM bank 3、8KB) から 1 byte 読出 | 0xE8 |
| `nes_peek16_ext($offset)` | int 0-8190 | USER_RAM_EXT から 2 byte LE 読出 | 0xE9 |
| `nes_poke_ext($offset, $byte)` | int, int | USER_RAM_EXT[$offset] = byte | 0xEA |
| `nes_pokestr_ext($offset, $string)` | int, string | USER_RAM_EXT に文字列の生バイトを bulk copy (内蔵 RAM 中継) | 0xEB |

USER_RAM ($0700-$07FF、256B) は内蔵 RAM 上の汎用バイト領域。配列要素 (1 要素 4B tagged zval + 配列毎 4B ヘッダ) より 4 倍コンパクト。USER_RAM_EXT (PRG-RAM bank 3、8KB) は更に大規模なテーブル / グリッド用 (例: tetris 系 example の盤面 lock 済タイル番号保存)。詳細は [`spec/02-ram-layout.md § USER_RAM`](./spec/02-ram-layout.md)。
...
| [`examples/tetris.php`](./examples/tetris.php) | フル機能テトリス v1 (参照用に残置): 7 種ピース (各色違い) + 4 回転 (A=時計回り / B=逆回転) + ライン消去 + スコア + GAME OVER → リスタート + レンガ壁 | shape table (28 entry × 16-bit) を `nes_pokestr` で USER_RAM bulk load、各 cell の lock 済タイル番号は USER_RAM_EXT (bank 3) に保存、PUSH START でランダム seed 確定 |
...
| [`examples/putint.php`](./examples/putint.php) | `nes_putint` で 5-char 右詰めスコア表示 | `nes_putint` の動作確認 |
| [`examples/shifttest.php`](./examples/shifttest.php) | `<<` `>>` の挙動確認 | shift 演算 |
| [`examples/multest.php`](./examples/multest.php) | `*` `/` `%` の動作確認 (16bit signed) | 算術 |
| [`examples/parentest.php`](./examples/parentest.php) | 括弧式 `(expr)` で precedence override | 構文 |
| [`examples/arrlit_test.php`](./examples/arrlit_test.php) | ネスト配列 `[[1,2],[3,4]]` リテラル | 配列 |
| [`examples/arrself.php`](./examples/arrself.php) | 配列の自己参照書込 (`$a[$i] = $a[$i] + 1`) | shared-pointer 確認 |
| [`examples/simple_random.php`](./examples/simple_random.php) | LFSR の最小デモ | `nes_rand` `nes_srand` |
...
`chr/make_font.php` が `chr/font.chr` (32KB = 4 × 8KB バンク) を生成します。そこを書き換えて `php chr/make_font.php` で再生成 → `make` で再ビルド。CHR-RAM 化以降、ROM に焼かれるのは**先頭 16KB のみ**で、PRG-ROM bank 1 (`CHRDATA` セグメント) に 4KB CHR セット × 4 として格納。起動時に先頭 8KB (セット 0-1) が CHR-RAM へ転送され、`nes_chr_bg($n)` / `nes_chr_spr($n)` (n = 0-3) が実行時に 4KB セットを再コピーします。

標準の内容:
- **セット 0** (起動時の BG pattern table): ASCII フォント — 基本 5×7、数字と大文字は 7×7 のチャンキーなアーケード風太字グリフ — に加えカスタムタイル: 日本国旗 (0x01-0x04)、テトロミノピース (0x05-0x0B)、レンガ壁 (0x0C)、elePHPant スプライト (0x10-0x15)、雲 (0x16-0x19)、? ブロック (0x1A-0x1D)
- **セット 1** (起動時の sprite pattern table): 上記の **インバース** (`nes_chr_bg(1)` で白抜き風)
- **セット 2-3**: セット 0-1 のコピー (プレゼン用に差し替える想定)
