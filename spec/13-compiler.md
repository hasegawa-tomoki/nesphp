...
## 対応文法 (2026-04-19 時点、M-A' + P1 + P2 + P3 + Q1-Q4 実装済)
...
| `TK_INT`   | `[0-9]+` / `0x[0-9a-fA-F]+` | 10 進 / 16 進、16bit signed narrow |
...
10. **`else` / `elseif` 未対応**、**`&&` `||` `!` / 単項 `-` 未対応**、**`<=` `>` `>=` 未対応**
11. **if / while / for のボディ**: `{ ... }` または単文どちらも可
12. **ネスト深さ**: backpatch stack 8 段、6502 HW stack 256B (for ネスト 1 段で 4B 消費)、CV table 32 エントリ
13. **対応 intrinsic**: `nes_cls` / `nes_put` / `nes_puts` / `nes_sprite` / `nes_chr_bg` / `nes_chr_spr` / `nes_bg_color` / `nes_palette` / `nes_attr` / `fgets`
