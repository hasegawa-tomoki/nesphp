...
| **L3S** | **PHP ソースを ROM に焼き、起動時に 6502 コンパイラが PRG-RAM に `zend_op 12B` / 4B tagged zval literal を emit する (value 3B + type 1B。文字列は (STR_POOL offset 2B, length 1B) を持つ)。zend_string 構造体は持たない** |
...
> **Note (literal 4B 移行):** 以下の 16B レイアウトは現在 host serializer (L3)
> 経路のみに適用される。on-NES コンパイラは PRG-RAM に **4B tagged literal**
> を emit する: `[v0, v1, v2, type]` — IS_STRING では v0-v1 = STR_POOL offset、
> v2 = length (8-bit、文字列 ≤ 255 バイト)。operand offset は literal/CV/TMP
> index × 4。
...
## 対応文法 (2026-07-19 時点。M-A' + P1-P4 + Q1-Q4 + R1-R3 + arrays/elseif/cmp-ops/mul-ops 実装済)
...
primary      ::= INT | STRING | CV | "true" | call_expr
             ...
             |   "[" (expr ("," expr)*)? "]"  (配列リテラル、ネスト可、最大 255 要素
                                              — INIT_ARRAY の capacity が 8-bit)
...
| `$a[idx]` | `ZEND_FETCH_DIM_R` (op1 = CV array, op2 = index、result = 新 TMP)。pool 要素は 4B tagged zval で格納されており、そのまま読み出す (16B zval は literal 専用) |
| `count($a)` | `ZEND_COUNT` (op1 = array、result = 新 TMP、IS_LONG = 要素数) |
| `$a[i] = v;` | `ZEND_ASSIGN_DIM` (op1=CV array, op2=index) + `ZEND_OP_DATA` (op1=value)。ハンドラは 2-op セットで array[i] に 4B tagged zval 書込、count = max(count, i+1) |
...
backpatch スタック (ZP 16B = 8 slot。**実効ネスト上限は 7 段** — 8 段目のネストブロックを開くと compile error、2026-07-19 検証済) に「後で埋めるべき op2 フィールドの PRG-RAM 絶対アドレス」を push する。ブロック終端で `cmp_bp_pop_patch` が現在の `CMP_OP_COUNT` をその位置に 16bit で書き込む。
...
$????-$7CFF  literals (4B tagged zval × num_lits、op_array 直後に memcpy)
$7D00-$7FFF  CMP_LIT_STAGE (768B = 4B × 最大 192 literal、compile 中のみのステージングバッファ)
```
...
- **literals**: cmp_finalize で CMP_LIT_STAGE の zval を $6010 + ops × 12 にバルク memcpy
...
- **ARR_POOL** (bank 1): runtime 中、配列ヘッダ 4B (count, capacity) + capacity × 4B tagged zval を追記型で alloc。GC 無し
...
zend_op の `op.var` フィールドには `slot * 4` を 16-bit で格納 — slot は 4B tagged value なので RAM オフセットそのもの。(2026-07-19 までは Zend 流に `slot * 16` を格納して resolver が `/4` していたが、4B literal 移行でこのシフトは廃止された。)

64 slot なら `slot * 4` は常に 1 byte に収まるが、`vm/nesphp.s` の `cv_addr_y` / `tmp_addr_y` ヘルパーは今もこのフィールドを (VM_PC, Y), (VM_PC, Y+1) から 16-bit で読んで base に加算する (`slot * 16` 時代の無害な名残)。res_cv / res_tmp / wr_cv / wr_tmp / assign_to_cv / incdec_cv_addr すべてここを通る。
...
7. **PRG-RAM 32KB (4 × 8KB bank)** がコンパイル出力の上限。bank 0 (op_array ~617 op + literal 4B tagged zval = 8KB)、bank 1 (ARR_POOL 8KB)、bank 2 (STR_POOL 8KB、文字列リテラルもここで PRG-RAM 常駐)、bank 3 (USER_RAM_EXT 8KB)
8. **CV 最大 64 スロット**、**TMP 最大 64 スロット** (文間でリセット、再利用可能)、**関数引数 ≤ 4**、**関数呼出ネスト無し** (call expr は `fgets` / `nes_btn` / `nes_rand` / `nes_peek` / `nes_peek16` / `nes_peek_ext` / `nes_peek16_ext` / `count` のみ)
9. **比較式は非連鎖** (`$a < $b < $c` は compile error)
10. **`!` / 単項 `-` 未対応**、**`^` (BW_XOR) 未対応**、**文字列連結 `.` 未対応**
11. **if / while / for のボディ**: `{ ... }` または単文どちらも可
12. **ネスト深さ**: backpatch stack は 8 slot だが**実効上限は 7 段ネスト** (8 段目の `{` は compile error、`ERR L<line> C<col>` と表示)、6502 HW stack 256B (for ネスト 1 段で 4B 消費)、CV table 64 エントリ
13. **対応 intrinsic** (合計 24 種): `nes_cls` / `nes_put` / `nes_puts` / `nes_putint` / `nes_sprite_at` / `nes_sprite_attr` / `nes_chr_bg` / `nes_chr_spr` / `nes_bg_color` / `nes_palette` / `nes_attr` / `fgets` / `nes_vsync` / `nes_btn` / `nes_rand` / `nes_srand` / `nes_peek` / `nes_peek16` / `nes_poke` / `nes_pokestr` / `nes_peek_ext` / `nes_peek16_ext` / `nes_poke_ext` / `nes_pokestr_ext`
