...
L3 host-serializer の ROM フォーマットは Zend 互換 16B zval を守る ([01-rom-format](./01-rom-format.md)) が、**動作中の VM が触るものは全て 4B**: on-NES コンパイラ (L3S、デフォルト経路) は literal を最初から 4B tagged に縮退した形で PRG-RAM に emit し、RAM 側の全スロット (スタック / CV / TMP / ARR_POOL) も 4B tagged value を保持する。2KB RAM でヒープなしの制約を守るため。

## 方針

- ROM (L3 host オラクル、`.host.ops.bin`): Zend 互換 16B zval (faithful)
- PRG-RAM literals (L3S): 4B tagged zval、レイアウト `[v0, v1, v2, type]` (コンパイル時に縮退、ただし type ID は Zend 互換)
- RAM (スタック / CV / TMP): 4B tagged value
...
### narrow のルール (L3 host-serializer 16B 形式)
...
- **L3 host 経路**: host の serializer は ROM (`.host.ops.bin`) に Zend 互換の 16B zval を書くだけで、narrow を事前に行わない。「ROM は Zend のレイアウトそのまま」という L3 方針の帰結。
- **L3S (現行デフォルト)**: on-NES コンパイラが**コンパイル時に** narrow する — literal は 4B tagged `[v0, v1, v2, type]` として PRG-RAM に置かれる (IS_STRING の場合: v0-v1 = STR_POOL offset、v2 = length)。VM の `resolve_op1` / `resolve_op2` はこれをそのまま読む (type はオフセット 3、[03-vm-dispatch](./03-vm-dispatch.md) 参照)。フェッチ時の narrow はもう存在しない。
...
**設計理由**: 配列 (`$a = [...]`) は 1 要素あたり 4 byte の tagged-zval オーバーヘッド
(+ 配列ごとに 4B header) があるため、バイト単位の大量データ (例: 56 byte の 28 entry
shape table) では依然 ~2 倍のメモリを食い、しかも ARR_POOL の bank 切替越しになる。
USER_RAM は常時マップの内蔵 RAM 上でオーバーヘッドゼロのバイトアクセスができる。
...
...-$7CFF    literals (op_array の直後に memcpy、各 4B tagged zval)
