...
- [ ] `examples/hello.php` に `<?php echo "HELLO, WORLD!";` が書かれている
...
### 検証 4: literals の 16B zval レイアウトが Zend 互換 (host-compile パスのみ)

16B zval レイアウトが残るのは host oracle の `.host.ops.bin` だけ — L3S on-NES compiler は代わりに 4B tagged literal を PRG-RAM に emit する ([13-compiler](./13-compiler.md))。
...
- literal 0 は 0x28 から始まる: `48 00 00 00 ...` = `value` (ROM 常駐 `zend_string` のオフセット 0x48)、zval オフセット 8 の `06 00 00 00` = `u1.type_info` の下位バイトが `IS_STRING(6)`
- literal 1 は 0x38 から始まる: `01 00 00 00 00 00 00 00` = `value.lval = 1`、`04 00 00 00` = `IS_LONG(4)`
...
`examples/chrdemo.php`: ボタン押下ごとにステートマシンが進み、`nes_chr_bg(0/1)` と
`nes_chr_spr(0/2)` を順に呼ぶ。同じテキストが通常 → インバース → 通常と変化し、
続いて sprite 側が CHR set 2 に切り替わって戻る。
