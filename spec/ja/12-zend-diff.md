...
- `constant`: literal 領域は 4KB を大きく下回る (host L3: ~48 zval × 16B、L3S: 最大 192 × 4B tagged) → 12-bit で十分
- `var`: スロット番号 × 4 (4B 移行前は × 16)、最大 64 スロット → 256 = 8-bit
...
host L3 フォーマットでは zval の物理レイアウトは **16B のまま変えていない**
ので、`value` union の下位 2 バイトだけを nesphp は見る。残りのバイトは
Zend 互換を維持するための「空白」です。(L3S ではこの空白ごと落とし、
literal は 4B tagged として PRG-RAM に置く — 改変 8 参照。)
...
4B literal 移行 (2026-07-19) 以降、L3S の on-NES コンパイラは **literals** も
PRG-RAM に 4B tagged (`[v0, v1, v2, type]` — type は offset 3) へ事前圧縮
した形で格納し、`resolve_op1` / `resolve_op2` はそのレイアウトを直接読みます。
fetch 時の「16B を読んで 4B に narrow する」変換は、いまや L3 host 形式に
対する概念上のものとしてだけ存在します。結果として:

- **Host oracle (`.host.ops.bin`)**: Zend の 16B zval レイアウトのまま
...
| zval 16B レイアウト | 維持 | 4B tagged literal に置換 (2026-07-19。それ以前の L3S は value 共用体の**意味**だけ変えて 16B を維持していた) |
...
に判断)。4B literal 移行 (2026-07-19) でギャップはさらに広がった: VM の
resolver は 4B tagged literal (type は offset 3) を期待するようになったので、
host 経路の 16B literal バイナリを実行するには、serializer が 4B literal を
吐くようにするか、resolver 側の互換 shim が追加で必要。今日の
`.host.ops.bin` は **op 列の oracle 専用**であり、VM にロード可能なイメージ
ではない。
...
| `zval` サイズ | 16B | L3 `.host.ops.bin`: 16B / L3S PRG-RAM literals: 4B tagged | ✅ (L3) / ❌ (L3S) |
...
1. `zend_op` の **12B 構造** (handler/lineno 削除 + 各 znode_op 4B→2B 圧縮) が Zend と同じフィールド順
   オフセットで ROM に並んでいる
2. `zval` 16B レイアウトが host L3 オラクル内で保たれている (中身の使い方は
   縮退。稼働経路の L3S は literal を 4B tagged に narrow 済み)
3. `zend_string` ヘッダの 24B レイアウトが保たれている (host L3 のみ。L3S は省略)
